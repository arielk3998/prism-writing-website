...
export async function getAllClients(): Promise<Client[]> {
  console.log('STUB: getAllClients called');
  return [];
}
...
export async function getCurrentUser(token?: string): Promise<User | null> {
  console.log('STUB: getCurrentUser called with token:', token ? 'provided' : 'none');
  return null;
}
...
export async function register(data: RegisterData): Promise<AuthResult> {
  console.log('STUB: register called');
  return {
    success: false,
    error: 'Register not implemented - stub version'
  };
}
