    ...
    <div className={`relative ${className}`}>
      ...
      >
        <div className="flex items-center space-x-3">
          <Globe className="w-5 h-5 text-safe-muted" />
          <div className="text-left">
            {selectedLanguage ? (
              <div>
                <span className="text-safe font-medium">{selectedLanguage.name}</span>
                {showNativeName && selectedLanguage.nativeName !== selectedLanguage.name && (
                  <span className="text-safe-muted ml-2">({selectedLanguage.nativeName})</span>
                )}
              </div>
            ) : (
              <span className="text-safe-muted">{placeholder}</span>
            )}
          </div>
        </div>
        <ChevronDown className={`w-5 h-5 text-safe-muted transition-transform ${isOpen ? 'transform rotate-180' : ''}`} />
      </button>
        ...
        <div className="absolute z-50 w-full mt-2 bg-white border border-gray-300 rounded-lg shadow-lg max-h-96 overflow-hidden">
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-safe-muted" />
              <input
                type="text"
          ...
          <div className="flex border-b border-gray-200">
            <button
              ...
              className={`flex-1 py-2 px-4 text-sm font-medium ${
                activeTab === 'popular'
                  ? 'text-safe-accent border-b-2 border-blue-600 bg-blue-50'
          ...
          <div className="max-h-64 overflow-y-auto">
            {activeTab === 'regions' ? (
              ...
              Object.entries(groupedByRegion).map(([region, regionLanguages]) => (
                <div key={region}>
                  <div className="px-4 py-2 bg-gray-50 text-xs font-semibold text-safe-muted uppercase tracking-wide">
                    {region}
                  </div>
            ...
            {filteredLanguages.length === 0 && (
              <div className="px-4 py-8 text-center text-safe-muted">
  ...
  return (
    <button
      ...
      className={`w-full px-4 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors ${
        isSelected ? 'bg-blue-50 text-safe-accent' : 'text-safe'
      }`}
    >
      <div className="flex items-center space-x-3">
        <div className="text-left">
          ...
          {showNativeName && language.nativeName !== language.name && (
            <div className="text-sm text-safe-muted">{language.nativeName}</div>
          )}
          {showSpeakers && (
      ...
      {isSelected && <Check className="w-5 h-5 text-safe-accent" />}
    </button>
  );
}
